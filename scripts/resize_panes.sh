...
# and tester (bottom) share the right column.
set -euo pipefail
...
[[ -z "$left_pane" || -z "$tr_pane" || -z "$br_pane" ]] && exit 0
...
# Two resizes are enough, and the order matters. The left column spans the
# whole window height by construction (it is never split vertically), so only
# its width is set here; sizing the right column's top pane then leaves the
# rest of the height to the bottom pane.
